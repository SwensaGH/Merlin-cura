...
def generate_nsi(source_path: str, dist_path: str, filename: str):
    ...
    print(instdir)
    dist_paths = [p.relative_to(dist_loc.joinpath("Merlin-Automation")) for p in sorted(dist_loc.joinpath("Merlin-Automation").rglob("*")) if p.is_file()]
    print(dist_loc)
    mapped_out_paths = {}
    for dist_path in dist_paths:
        if "__pycache__" not in dist_path.parts:
            ...
            if out_path not in mapped_out_paths:
                mapped_out_paths[out_path] = [(dist_loc.joinpath("Merlin-Automation", dist_path), instdir.joinpath(dist_path))]
            else:
                mapped_out_paths[out_path].append((dist_loc.joinpath("Merlin-Automation", dist_path), instdir.joinpath(dist_path)))
    ...
    nsis_content = template.render(
        # app_name = f"Merlin Printer {os.getenv('CURA_VERSION_FULL')}",
        app_name = "Merlin Printer 1.0.0",
        main_app = "Merlin-Automation.exe",
        ...
        version_minor='1',
        version_patch='0',
        company = "Merlin Automataion Solutions",
    ...
    print(dist_loc.joinpath("UltiMaker-Cura.nsi"))
